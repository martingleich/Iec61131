...
namespace SourceGenerator
{
	public class TokenDescriptionWithValue : TokenDescription
	{
		...
		public override Code ToCode()
		{
			...
			cw.StartBlock();
			cw.WriteLine($"public {Name}({ValueType} value, string generating, int startPosition, IToken? leadingNonSyntax) : base(value, generating, startPosition, leadingNonSyntax) {{ }}");
			if(DefaultValue != null)
				cw.WriteLine($"public readonly static Func<int, {Name}> Synthesize = startPosition => SynthesizeEx(startPosition, {DefaultValue});");
			cw.WriteLine($"public static {Name} SynthesizeEx(int startPosition, {ValueType} value) => new {Name}(value, \"\", startPosition, null);");
